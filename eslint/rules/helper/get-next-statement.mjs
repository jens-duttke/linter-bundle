/**
 * @file ESLint helper function for retrieving the next statement after a given node.
 */

/**
 * Gets the next statement after the given node, supporting switch cases.
 *
 * @param {import('eslint').Rule.RuleContext} context - The ESLint rule context.
 * @param {import('eslint').Rule.Node} node - The current variable declaration node.
 * @returns {import('eslint').Rule.Node | undefined} The next statement if found, otherwise undefined.
 */
export function getNextStatement (context, node) {
	const nextNode = context.sourceCode.getTokenAfter(node);
	...
	if (nextNode) {
		...
		if (Array.isArray(parentBody)) {
			...
			if (index !== -1 && index < parentBody.length - 1) {
